...
int main() {
    ...
    if(R <= 0)
    {
        while(R <= 0)
        {
            std::cout << "Неверная сторона. Введите другую:"<< std::endl; std::cin >> R;//Проверка правильности введенной стороны
        }
    }
    ...
    if(X1 <= X + R)
    {
        if(X1 >= X)
        {
            if(Y1 <= Y + R)
            {
                if(Y1 >= Y) std::cout <<"Точка находится в квадрате"<< std::endl; //Проверка и вывод результата
            }

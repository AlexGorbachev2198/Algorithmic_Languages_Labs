...
int main() {
    ...
    if(R <= 0)
    {
        while(R <= 0)
        {
            std::cout << "Неверный радиус. Введите другой радиус:"<< std::endl; std::cin >> R;
        }
    }
    ...
    std::cout << "Введите Y точки:0"<< std::endl; std::cin >> Y1;
    if(X1 <= X + R)
    {
        if(X1 >= X - R)
        {
            if(Y1 <= Y + R)
            {
                if(Y1 >= Y - R) std::cout <<"Точка находится в окружности"<< std::endl;
            }
